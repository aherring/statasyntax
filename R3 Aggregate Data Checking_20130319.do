*Start with uncleaned student-level AH dataset.*
...
recode r_aframb (1=0) if r_multi==1
recode r_nativeal (1=0) if r_multi==1
recode r_asianind (1=0) if r_multi==1
recode r_white (1=0) if r_multi==1
recode r_sphislat (1=0) if r_multi==1
recode r_oth (1=0) if r_multi==1
...
recode `var' (1=0) if drophighschool==0
}
...
foreach var of varlist srgrades1-em_attempt88 {
rename `var' `var'_AH
}

save "qualtricstestdata_forcollapse_cleaned"

*Collapse data to school level.*
collapse year-em_self srgrades1-em_attempt88, by(School_Short)
save "qualtricstestdata_school_cleaned"
...
use "sqltestdata_school.dta", clear
sort School_Short
save "sqltestdata_school_cleaned.dta"

*Merge AH and MN datasets.*
use "qualtricstestdata_school_cleaned", clear
sort School_Short
merge 1:1 School_Short using "sqltestdata_school_cleaned.dta", assert(match)
save "mergedtestdata_school.dta"

*Display discrepancies by variable, listing each observation for which value does not*
*match for each variable.                                                            *
foreach var of varlist year-em_attempt88 {
list responseid `var' `var'_AH if `var' != `var'_AH, abbreviate(15)
}

*Collapse data to school level.*
collapse year-em_self srgrades1-em_attempt88, by(Grade_Segment)
save "qualtricstestdata_gradesegment_cleaned"

*Prepare grade segmentation data from Python for checking after combining runprop and* 
*runmean Python files, concatenating School_Short and Grade_Level, and importing     *
*prepared file into Stata.                                                           *
use "sqltestdata_gradesegment.dta", clear
sort Grade_Segment
save "sqltestdata_gradesegment_cleaned.dta"

*Merge AH and MN datasets.*
use "qualtricstestdata_gradesegment_cleaned", clear
sort Grade_Segment
merge 1:1 Grade_Segment using "sqltestdata_gradesegment_cleaned.dta", assert(match)
save "mergedtestdata_gradesegment.dta"

*Display discrepancies by variable, listing each observation for which value does not*
*match for each variable.                                                            *
foreach var of varlist year-em_attempt88 {
list Grade_Segment `var' `var'_AH if `var' != `var'_AH, abbreviate(15)
}
